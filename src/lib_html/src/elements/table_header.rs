use super::*;
pub struct TableHeader<'re> {
    pub classes: Classes<'re>,
    pub id: Option<&'re str>,
    pub children: Vec<'re, AnyElement<'re>>,
    pub(crate) arena: &'re Bump,
    pub(crate) pre_render_hook: PreRenderHookStorage<'re, TableHeader<'re>>,
}
impl BuiltinHtmlElement for TableHeader<'_> {
    derive_class!();
    derive_id!();
}
derive_pre_render_hooks!('re, TableHeader<'re>);
impl<'re> TableHeader<'re> {
    pub fn child(mut self, child: TableRow<'re>) -> TableHeader<'re> {
        self.children.push(child.into_any_element(self.arena));
        self
    }
}
impl<'re> SimpleElement<'re> for TableHeader<'re> {

...
///
/// If the `cite` attribute is present, it must be a valid URL potentially surrounded by spaces. User agents may allow users to follow such citation links,
///     but they are primarily intended for private use (e.g., by server-side scripts collecting statistics about a site's use of quotations), not for readers.
///
...
pub fn q(arena: &Bump) -> Quote<'_, WithoutChild> {
    Quote {
        classes: Classes::new_in(arena),
        id: None,
        children: Vec::new_in(arena),
        cite: CiteAttribute::new_in(arena),
        arena,
        pre_render_hook: PreRenderHookStorage::new_in(arena),
        has_child: PhantomData,
    }
...
pub fn mark(arena: &Bump) -> Mark<'_, WithoutChild> {
    Mark {
        classes: Classes::new_in(arena),
        id: None,
        children: Vec::new_in(arena),
